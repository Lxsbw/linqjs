...
describe 'Group 2:', () ->
  ...
  test 'thenBy & thenByDescending', () ->
    ...
    list = new Linq(persons)
    orderByID = list.orderByDescending((x) -> x.ID).toArray()
    thenByAge = list.orderByDescending((x) -> x.ID).thenBy((x) -> x.Age).toArray()
    thenByName = list.orderByDescending((x) -> x.ID).thenBy((x) -> x.Age).thenByDescending((x) -> x.Name).toArray()
...
describe 'Group 3:', () ->
  ...
  test 'cloneDeep fallback preserves complex values', () ->
    ...
    try
      ...
      clone = toolObj.cloneDeep(source)
      cloneMapKey = Array.from(clone.map.keys())[0]
      cloneSetValue = Array.from(clone.set.values())[0]

      expect(clone).not.toBe(source)
      expect(clone.self).toBe(clone)
      expect(clone.sparse).toHaveLength(3)
      expect(0 of clone.sparse).toBeFalsy()
